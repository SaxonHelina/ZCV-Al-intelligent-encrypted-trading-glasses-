...
def checkout_dart(info: DartLibInfo):
    ...
    if not os.path.exists(clonedir):
        ...
        subprocess.run([GIT_CMD, '-c', 'advice.detachedHead=false', 'clone', '-b', info.version, '--depth', '1',
                        '--filter=blob:none', '--sparse', '--progress', DART_GIT_URL, clonedir], check=True)
        ...
        subprocess.run([GIT_CMD, 'sparse-checkout', 'set', 'runtime', 'tools', 'third_party/double-conversion'],
                ...
                if os.path.exists(utils_path):
                    with open(utils_path, "r+") as f:
                        ...
                        if r"match_against('^MAJOR (\d+)$', content)" in content:
                            ...
                            content = content.replace(" ' awk ", " r' awk ").replace("match_against('",
                                                                                     "match_against(r'").replace(
                                "re.search('", "re.search(r'")
                            if "import imp\n" in content:
                                content = content.replace("import imp\n", imp_replace_snippet).replace(
                                    "imp.load_source", "load_source")
                            f.seek(0)
                            f.truncate()
                            f.write(content)
            # make version
            subprocess.run([sys.executable, 'tools/make_version.py', '--output', 'runtime/vm/version.cc', '--input',
                            'runtime/vm/version_in.cc'], cwd=clonedir, check=True)
        else:
...
def cmake_dart(info: DartLibInfo, target_dir: str):
    ...
    subprocess.run([CMAKE_CMD, '-GNinja', '-B', builddir, f'-DTARGET_OS={info.os_name}', f'-DTARGET_ARCH={info.arch}',
                    f'-DCOMPRESSED_PTRS={1 if info.has_compressed_ptrs else 0}', '-DCMAKE_BUILD_TYPE=Release',
                    '--log-level=NOTICE'],
