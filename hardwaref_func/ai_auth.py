...
def capture_iris_image():
    ...
    print("Please position your eye in front of the camera.")
    while True:
        ret, frame = cap.read()
        if not ret:
...
def authenticate_user(iris_features, stored_features):
    """Compare the captured iris features with stored features."""
    distance = np.linalg.norm(iris_features - stored_features)
    threshold = 1000  # This threshold should be determined experimentally
...
if __name__ == "__main__":
    stored_features = np.load("stored_iris_features.npy")  # Load pre-stored features
    ...
    if eye_image is not None:
        ...
        if iris_circle is not None:
            ...
            authenticate_user(iris_features, stored_features)
